	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//detail
        	...
        	if(id == null) {
             ...
             //잘가~
             forward("infodetail.jsp", req, resp);
        ...
        //수정
			...
			forward("infoupdate.jsp", req, resp);
		...
		//수정완료
			...
			forward("infoupdateAf.jsp", req, resp);
		...
		//글쓰기	
			...
			req.setAttribute("id", id);
			forward("infowrite.jsp", req, resp);
		...
		//글쓰기 완료
			...
			req.setAttribute("isS", isS);
			forward("infowriteAf.jsp", req, resp);
		...
		//페이징처리
			/*
			 * }else if (work.equals("pageNum")) {
			 * 
			 * //검색 String searchWord = req.getParameter("searchWord"); String choice =
			 * req.getParameter("choice");
			 * 
			 * System.out.println("pageNum1 choice="+choice);
			 * System.out.println("pageNum1 searchWord="+searchWord);
			 * 
			 * if(choice == null || choice.equals("")){ choice = "sel"; }
			 * 
			 * //검색어를 지정하지 않고 choice가 넘어왔을 때 if(choice.equals("sel")){ searchWord =
			 * "";//null->""로 변경해줌(=new String()) } if(searchWord == null){ searchWord = "";
			 * choice = "sel";//select에서 "------선택"으로 돌아가기 }
			 * 
			 * int len = dao.getAllBbs(choice, searchWord);
			 * 
			 * System.out.println("pageNum len:"+len);
			 * System.out.println("pageNum2 choice="+choice);
			 * System.out.println("pageNum2 searchWord="+searchWord); String spageNumber =
			 * req.getParameter("pageNumber"); int pageNumber = 0;//현재페이지 if(spageNumber !=
			 * null && !spageNumber.equals("")){//하라미터가 넘어왔을때 pageNumber =
			 * Integer.parseInt(spageNumber); }
			 * System.out.println("pageNum pageNumber:"+pageNumber);
			 * 
			 * List<InfoDto> list = dao.getBbsPagingList(choice, searchWord, pageNumber);
			 * 
			 * req.setAttribute("list", list); req.setAttribute("len", len);
			 * req.setAttribute("choice", choice); req.setAttribute("searchWord",
			 * searchWord); req.setAttribute("pageNumber", pageNumber);
			 */
