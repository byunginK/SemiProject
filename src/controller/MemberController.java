...
public class MemberController extends HttpServlet {
	...
	//	System.out.println(id);	
	  ...
	  if(req.getParameter("new_pwd") != null && !req.getParameter("new_pwd").equals("")){
	      ...
	      if(isS) {
			       out.print("<script> alert('정보수정에 성공했습니다.'); location.href = 'MyPage.jsp?id="+ id + "';</script>");
			       out.flush();
	      }else {
		          out.print("<script> alert('정보수정에 실패했습니다. 해당 현상이 지속되면 문의해주세요(문의전화 02-233-5555)'); location.href = 'index.jsp';</script>"); out.flush(); 
		     }		
		 }
    ...
    if(order_c > 0) {
	    out.print("<script> alert('구매 중인 제품이 있습니다. 확인 부탁드립니다.'); location.href ='newMyPage.jsp?id="+id + "';</script>");    
	    out.flush();
    }else {
	     dao.DeleteMem(id);
