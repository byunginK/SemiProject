				...
				<table>
					...
					<col width="50"><col width="80"><col width="50"><col width="120">
						...
						<tr class='listtr'>
							...
							<td id="name" onclick="goPDetail(<%=dto.getItem_seq()%>)">
								<dl>
		...
		price = price-itemprice;
	 	...
	 	$.ajax({
			...
			success:function(obj){
				if(!obj.b){
					swal("","정상적으로 수량이 수정되지 않았습니다","error");
...
function delbtn() {
	...
	if(dseq==""){
		swal("","상품 선택 후 삭제를 해주시기 바랍니다","error");
		...
		$.ajax({
			...
			success:function(obj){
				if(!obj.b){
					swal("","정상적으로 선택 상품이 삭제되지 않았습니다","error");
					/* alert("정상적으로 선택 상품이 삭제되지 않았습니다"); */
				}else{
					swal("","정상적으로 삭제 되었습니다","error");
...
function payfun() {
	...
	if(dseq==""){
		swal("","상품 선택 후 결제 해주시기 바랍니다","error");
...
function goPDetail(data) {
	//console.log(data);
	location.href = "productDetail?work=product&seq="+data;
}
</script>
