...
public class blistDao {
	...
	public List<blistDto> getbList(){
		String sql = " SELECT SEQ, TITLE, CONTENTS, PART_SEQ, ID, SECRET, WDATE, READCOUNT FROM FIVE_QNA ORDER BY SEQ DESC ";
		...
		try {
			...
			while(rs.next()) {
				...
				blistDto dto = new blistDto(rs.getInt(i++), rs.getString(i++),rs.getString(i++),
											rs.getInt(i++), rs.getString(i++),rs.getInt(i++), rs.getString(i++),rs.getInt(i++));
				list.add(dto);
			}
	...
	public List<blistDto> getbList(int pageNumber){
		String sql = " SELECT SEQ, TITLE, CONTENTS, PART_SEQ, ID, SECRET, WDATE, READCOUNT " + 
				" FROM " + 
				" ( " + 
				" SELECT ROW_NUMBER()OVER(ORDER BY SEQ DESC) AS RNUM, " + 
				"	SEQ, TITLE, CONTENTS, PART_SEQ, ID, SECRET, WDATE, READCOUNT " + 
				" FROM FIVE_QNA " + 
				" ORDER BY SEQ DESC " + 
				" ) " + 
				" WHERE ? <= RNUM AND RNUM <= ? ";
	...
	public List<blistDto> getbList(int pageNumber,String need, String fneed){
		...
		try {
			...
			while(rs.next()) {
				...
				blistDto dto = new blistDto(seq,title,contents,part_seq,id,secret,wdate,readcount);
	...
	public boolean writeOne(blistDto dto) {
		String sql = " INSERT INTO FIVE_QNA(SEQ, TITLE, ID, PART_SEQ, CONTENTS, SECRET, WDATE, UDATE, READCOUNT ) "
				+ " VALUES (FIVE_QNA_SEQ.NEXTVAL, ?, ?, ?, ?, ?, SYSDATE,SYSDATE,0 ) ";
	...
	public blistDto updateReadcount(blistDto dto) {
		String sql = " UPDATE FIVE_QNA SET READCOUNT = READCOUNT + 1 WHERE SEQ = ? ";
		String sql1 = " SELECT READCOUNT FROM FIVE_QNA WHERE SEQ = ? ";
		...
		try {
			conn = DBConnection.getConnection();
			psmt = conn.prepareStatement(sql);
			System.out.println("1/9 updateReadcount success ");
			
			psmt.setInt(1, dto.getSeq());
			count = psmt.executeUpdate();
			System.out.println("2/9 updateReadcount success ");
			
			if(count>0) {
				psmt.clearParameters();
				
				psmt = conn.prepareStatement(sql1);
				System.out.println("3/9 updateReadcount success ");
				
				psmt.setInt(1, dto.getSeq());
				
				rs = psmt.executeQuery();
				System.out.println("4/9 updateReadcount success ");
				...
				System.out.println("5/9 updateReadcount success ");
			}
